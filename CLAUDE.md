...
- **デザイン**（2026-09-02 モダンミニマルへ全面刷新）: モノクロ基調＝白 `#FFFFFF`・インク `#1A1A1A`・グレー `#6B6B6B`・ヘアライン `rgba(0,0,0,.10)`、アクセントは蛍光黄緑 `#EAF33E` を極小面積のみ（グラフの「今日」の点・選択中ナビと期間切替の下線・習慣チェックの完了塗り。文字色には使わない）。フォント Noto Sans JP（和文）／ Inter（英字・数値）。カードの塗り・影・色つき左罫・指標別8色は廃止し、1px ヘアライン＋余白で構成。スクロール連動の出現演出・bounce・scale-snap も廃止。参考事例＝toshiyukihashimoto.jp / growth-next.com。旧「木骨（きこつ）」パレット（生成り・木・墨）は廃止
